...
class TelegramService {
    constructor() {
        this.botToken = process.env.TELEGRAM_BOT_TOKEN || "8378067144:AAFeP7zGV-6HZMXRXe2tXGE8euP7kBLeUak";
        this.primaryChannelId = process.env.TELEGRAM_CHANNEL_ID || "-1003001236281";
        ...
        // Force IPv4 for Telegram connections with better timeout handling
        const botOptions = { 
            polling: false,
            request: {
                agentOptions: {
    ...
    async splitVideoFile(videoPath, chunkSize) {
        ...
        // Generate unique chunk prefix using timestamp and random number
        const uniqueId = Date.now() + "-" + Math.random().toString(36).substr(2, 9);

        for (let i = 0; i < numChunks; i++) {
            const chunkPath = path.join(tempDir, `chunk_${uniqueId}_${i}.bin`);
        ...
        console.log(`📦 Created ${numChunks} unique chunks with ID: ${uniqueId}`);
        return chunks;
    }
    ...
    async uploadLargeVideoToChannel(videoPath, originalName, videoId, checksum, channelId) {
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            while (!success && retryCount < maxRetries) {
                    // Debug: verify chunk file exists
                    console.log(`🔍 Attempting to send chunk: ${chunkPath}`);
                    const chunkExists = await fs.pathExists(chunkPath);
                    console.log(`📁 Chunk exists: ${chunkExists}`);
                    if (!chunkExists) {
                        throw new Error(`Chunk file not found: ${chunkPath}`);
                    }

                try {
                    result = await this.bot.sendDocument(channelId, chunkPath, {
                        caption: caption
                    });
                    success = true;
                    ...
                    retryCount++;
                    console.error(`❌ Error uploading chunk ${i + 1}/${chunks.length}, attempt ${retryCount}:`, error.message);
                    ...
                    // If max retries reached, throw error to stop the upload
                    if (retryCount >= maxRetries) {
                        console.error(`💥 Failed to upload chunk ${i + 1}/${chunks.length} after ${maxRetries} attempts`);
                        
                        // Clean up remaining chunk files
                        for (let j = i; j < chunks.length; j++) {
                            try {
                                await fs.remove(chunks[j]);
                            } catch (cleanupError) {
                                console.warn('Error cleaning up chunk:', cleanupError.message);
                            }
                        }
                        
                        throw new Error(`ETELEGRAM: ${error.message}`);
                    }
                }
            }
